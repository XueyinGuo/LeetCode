...
public class ManacherII {

    public static void main(String[] args) {

        for (int i = 0; i < 1; i++) {
            String randomString = "aba1233321rtiwoiewie";
            String manacher = manacher(randomString);
...
//            max = Math.max(radius[i] - 1, max);
        ...
        int realCenter = startIndex / 2;
        StringBuffer sb = new StringBuffer();
        int start = realCenter - (max / 2);
        if (max % 2 != 0){

            for (int i = start; i < start +  max / 2 * 2 + 1; i++) {
                sb.append(s.charAt(i));
            }

        }else{
            for (int i = start; i <start +  max / 2 * 2; i++) {
                sb.append(s.charAt(i));
            }
        }
        return sb.toString();
