...
public class L1744_FavoriteCandy {

    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        int length = queries.length;
        boolean[] ans = new boolean[length];
        long[] preCandySum = new long[candiesCount.length];
        preCandySum[0] = candiesCount[0];
        for (int i = 1; i < candiesCount.length; i++) {
            preCandySum[i] = preCandySum[i - 1] + candiesCount[i];
        }
        for (int i = 0; i < length; i++) {
            int type = queries[i][0];
            int day = queries[i][1] + 1;
            int cap = queries[i][2];
            /* 吃糖果的区间，从 day颗糖果开始----day*cap 之间 是在第 day天可以吃哪些糖 */
            long l1 = day;
            long r1 = (long) day * cap;
            /* 某一类型糖果可以在哪些天被吃到，每天吃一颗的话 从 sum[type - 1]+1天开始可以吃到，可以吃到 sum[type] 那一天  */
            long l2 = type == 0 ? 1 : preCandySum[type - 1] + 1;
            long r2 = preCandySum[type];
